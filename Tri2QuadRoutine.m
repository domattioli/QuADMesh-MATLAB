...
for iLayer	= Domain.nLayers:-1:1                   % Work outward.
    ...
    %%% For real-time plotting.
     subDomain.plot;
    % For each path in layer, advance along the vertices.
    for iPath	= 1:size(Path,2)
        ...
        for iPathVertID = 2:length(PathVertIDs)-1   % Begin at source vert.
            %% 1. Advance along verts in iPath, identifying every-other edge.
...
%             subDomain.plotPoint(VertIDs(PathVertIDs(iPathVertID+1)),'Color','b');   % Next vertex along path.
            ...
            % Note: this can be avoided if vert2edges are in a CW/CCW arrangement.
...
%                     subDomain.plotEdge(removeEdgeIDs(i_removeEdgeIDs-1),'Color','w');%drawnow;%pause(.01)
...
%             pause(.1)
            %% 2. Remove All Indexed Edges To Create Quads.
            
            subDomain.plotEdge(removedEdgeIDs,'Color','g');drawnow;%pause(.01)
...
toc
%%  3. Build Global Quad CHILmesh Object for Post-Processing.
