function MESH = distmesh2d(PTS,phi,MeshFun,xyzFun,hmin,sb,pH,viewStatus)
...
%--------------------------------------------------------------------------
ttol    = .5;           % Specifies how far the points can move (relatively) before a retriangulation
...
p = createInitialPointList(PTS,hmin); drawnow

%--------------------------------------------------------------------------
% Remove points outside the region, apply the rejection method
%--------------------------------------------------------------------------
p = rejectionMethod(p,phi.f,MeshFun,geps); drawnow
...
for k = 1:niter
    ...
    p(in,:) = projectBackToBoundary(phi,p(in,:),geps);
