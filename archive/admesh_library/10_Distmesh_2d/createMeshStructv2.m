function MESH = createMeshStructv2(t,p,MESH,PTS,xyzFun)
...
%---------------------------------------------------------------------
% If constraints exist in PTS, pair it up with updated mesh 
%---------------------------------------------------------------------
if ~isempty(PTS.BC)
    
    %-----------------------------------------------------------------
    % If open ocean boundaries exist, complete new node strings
    % NOTE: This BC was already appended in GetMESHConstraints
    %-----------------------------------------------------------------
    if any(ismember([MESH.BC.num],-1))
        
        % Obtain open ocean boundary data
        [NOPE,~,~,NBDV] = ExractNodeString(t,p,PTS,-1);
        ...
        for i = 1:NOPE
            MESH.BC(ix(i)).nodes = NBDV{i}'; % Assign node string
        end
        
    end
    
    %----------------------------------------------------------------
    % Find any internal and/or external nodes
    %----------------------------------------------------------------
    if any(ismember([PTS.BC.num],[0 2 10 12 20 22 30 1 11 21]))
        
        % BC flag
        type = [0 2 10 12 20 22 30 1 11 21];
        
        % Loop over each BC in PTS and find matches
        for k = 1:length(type)
            
            % Find index in mesh structure to store node string
            ix = find(ismember([PTS.BC.num],type(k)));
            
            if ~isempty(ix)
                
                % node string
                [NOPE,~,~,NBDV] = ExractNodeString(t,p,PTS,type(k));
                
                NOPE
                
                for i = 1:NOPE
                    MESH.BC(end+1).nodes =  NBDV{i}'; % Assign node string
                    MESH.BC(end).num     = PTS.BC(ix(k)).num;
                    MESH.BC(end).type    = PTS.BC(ix(k)).type;
                end
