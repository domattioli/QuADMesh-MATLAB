function h_ic = EnforceBoundaryConditions(h_ic,X,Y,D,IB,PTS,hmax,hmin,sb)
...
%--------------------------------------------------------------------------
if isempty(PTS.BC); return; end

%--------------------------------------------------------------------------
% Enforce open ocean boundary
%--------------------------------------------------------------------------
h_ic(IB) = hmax;
% % Are there any open ocean boundaries?
% ix = find(ismember([PTS.BC.num],-1));
% 
% if ~isempty(ix);
%     
...
%         POLY = create_polygon_structure(PTS.BC(ix(k)).points,hmax);
%         
%         for i = 1:size(POLY.x,1)
%             
%             % Find indices that fall within polygon
%             %IN = pointsInPolygons([POLY.x(i,:)',POLY.y(i,:)'],X(ind),Y(ind));
...
%             h_ic(ind(IN)) = hmax;
%             
%             %IN = pointsInPolygons([POLY.Circ.x(i,:)',POLY.Circ.y(i,:)'],X(ind),Y(ind));
...
if ~isempty(ix);
    ...
    for k = 1:length(ix)
        ...
        % Create Polygon Structure
        POLY = create_polygon_structure(PTS.BC(ix(k)).points);
...
%--------------------------------------------------------------------------
% Are there any internal boundaries?
% ix = find(ismember([PTS.BC.num],18));
% 
% if isempty(ix);
%     
%     % Find band
