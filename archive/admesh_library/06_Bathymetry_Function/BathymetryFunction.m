function h0 = BathymetryFunction(h0,D,X,Y,Z,s,hmin,hmax,delta,Settings,sb)
...
if strcmp(Status,'On') && ~isempty(Z) % If true, compute bathymetry function
    ...
    %------------------------------------------------------------------------------
    sb.setText('Computing the gradient of the bathymetry...')
        
    gradBx(3:LY-2,3:LX-2) = ( 1*Z(3:LY-2,1:LX-4) + -8*Z(3:LY-2,2:LX-3) + ...
        8*Z(3:LY-2,4:LX-1) + -1*Z(3:LY-2,5:LX) )/(12*delta);
    
    gradBy(3:LY-2,3:LX-2) = ( 1*Z(1:LY-4,3:LX-2) + -8*Z(2:LY-3,3:LX-2) + ...
        8*Z(4:LY-1,3:LX-2) + -1*Z(5:LY,3:LX-2) )/(12*delta);
