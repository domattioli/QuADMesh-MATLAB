classdef CHILmesh% < CHILgraph
    ...
    %       Ct	= centroid(CM);                 % Shows diagonals as matrix.
    %       plot(CM);                           % Plots mesh.
    %       plot(Ct(:,1),Ct(:,2),'r*');
    %
    ...
    %       fe  = boundaryEdges(CM)'            % Boundary edges.
    %       plot(CM);                           % Plots mesh.
    ...
    %       BOUNDARYCONDITION	- The boundary condition(s) of the mesh.
    %     	MAPPING           	- The current geographical mapping system.
    %       PLOT                - Ploting object for the mesh.
    %
    %
    %   CHILMESH was developed by Ethan J. Kubatko, Omar El Khoury, Dominik
    %   Mattioli, and Dustin J. West in the Computational Hydrodynamics &
    %   Informatics Laboratory (CHIL) at The Ohio State University in 2017,
    %   with funding in part by Aquaveo and U.S. Army Corps of Engineers.
    %   Some aspects of this class are inspired by:
...
properties
    ...
    methods
        ...
        function CM	= CHILmesh(varargin)            % Class Constructor.
            ...
            switch nargin
                case {0,1}                          % Grid or triangulation given.
                    if nargin == 0                  % Create an input if none exist.
                    ...
                    switch class(varargin{1})       % Now 1 input exists.
                        case {'triangulation',...	% Given grid as Triangulation.
                            ...
                            CM.GridName	= 'MATLAB Triangulation';
                ...
                case {2,3}                          % Given points, connectivity lists & boundary conditions.
                    % Accommodate for 2 args: TR and BCs
                    if nargin == 2                  % Parse into 3 inputs.
                        varargin{3} = [];           % Temporary.
                    end
            ...
            CM  = buildAdjacencies(CM);             % Build mesh adjacency lists.
            CM  = meshLayers(CM);                   % Identify mesh layers.
        end
        ...
        function [CM,BoundaryCondition]	= readFort14(CM,FILENAME)
            ...
            msgbox('Mesh loaded','Import Completed');
        end
        ...
        function CM	= buildAdjacencies(CM)
            ...
            % Check I/O arguments.
            [iT,iQ]	= elemType(CM);                 % Identify tris, quads.
            if isempty(iQ)                          % Do nothing.
                mixedElemMesh	= false;            % Triangular mesh.
            else                                    % Tris may exist in the
                mixedElemMesh	= true;             % quad/mixed-elem mesh.
            ...
            if mixedElemMesh                        % Account for 4th vertex.
            ...
            % Intersect edge-to-vertex and vertex-to-element tables to get
            % the edge-to-element table.
            edge2Elem	= [...
        ...
        function CM	= meshLayers(CM)
            ...
            EL	= edge2Vert(CM);                    % Edge list.
            ENL	= edge2Elem(CM);                    % Edge-neighbors list.
            ...
            CM.Layers	= struct('OE',[],'IE',[],'OV',[],'IV',[]);
            iL  = 1;                                % Current Layer.
            while any(ENL(:) > 0)                   % Until all elems flagged.
                % Identify outer edges of iL.
                if iL == 1	% Boundary of CM defined 1st layer's outer vertices.
                    % Check if open ocean boundary(ies) exist.
                    EdgeIDs	= boundaryEdges(CM);    % Index boundary edges.
                    %                     if open_bound_exists         	% Open boundary exists.
                    %                         EdgeIDs(iE_open) = false;	% Flag open boundary edges.
                    %                     end
                    CM.Layers.OV	= [CM.Layers.OV,...
                        {VertIDs(ismember(VertIDs,EL(EdgeIDs,:)))}];
                    
                else	% OV of iL defined by edges with only 1 neighbor.
                    % Check if open ocean boundary(ies) exist.
                    EdgeIDs	= sum(ENL > 0,2) == 1;	% Outer edges of layer.
                    %                     if open_bound_exists         	% Open boundary exists.
                    %                         EdgeIDs(iE_open) = false;	% Flag open boundary edges.
                    %                     end
                    CM.Layers.OV	= [CM.Layers.OV, unique(EL(EdgeIDs,:))];
                end
                
                % Outer Elements of iL are the neighbors of EdgeIDs.
                CM.Layers.OE	= [CM.Layers.OE,...
                    {ElemIDs(ismember(ElemIDs,ENL(EdgeIDs,:)))}];
                
                % Flag edges in EN used by OE.
                ENL(ismember(ENL,CM.Layers.OE{end}))	= NaN;
                
                % Get all edges associated with iL's OV.
                EdgeIDs	= sum(ismember(EL,CM.Layers.OV{end}),2) > 0;
                
                % Inner Elements of iL are the neighbors of EdgeIDs.
                CM.Layers.IE	= [CM.Layers.IE,...
                    {ElemIDs(ismember(ElemIDs,ENL(EdgeIDs,:)))}];
                
                % Flag used edges in EN and E;
                ENL(ismember(ENL,CM.Layers.IE{end}))	= NaN;
                EL(ismember(EL,CM.Layers.OV{end}))      = NaN;
                
                % Inner Vertices of iL are the vertices of edges defined by EdgeIDs.
        ...
        function [X,Y,Z]	= centroid(CM,ElemIDs)
            ...
            [iT,iQ]	= elemType(CM,ElemIDs);         % Tris, quads in mesh.
            ...
            [x,y,z]	= elemCoordinates(CM,ElemIDs);  % Coordinates of elems.
        ...
        function VertIDs	= diagonals(CM,ElemIDs)
            ...
            if nargin == 1                          % If no edges were
                ElemIDs = zeros(CM.nElems,1,'uint32');% inputted then
                ElemIDs(:)	= 1:CM.nElems;          % select all of them.
            end
            
            % Diagonals of quadrilateral(s) only.
            iT  = elemType(CM,ElemIDs);             % Identify triangles.
            if length(iT) == CM.nElems              % Mesh is only tris.
                return                              % End function.
            else
                iT  = iT.*2;                        % Adjust indices to
                iT  = [iT-1; iT];                   % all triangles.
            end
            
            % Identify diagonals.
            idx0    = ElemIDs == 0;                 % Search for zeros.
            VertIDs	= zeros(length(ElemIDs)*2,2,'uint32');
            VertIDs(2:2:end,:)	= CM.ConnectivityList(ElemIDs(~idx0),[1 3]);
            VertIDs(3:2:end,:)	= CM.ConnectivityList(ElemIDs(~idx0),[2 4]);
            VertIDs(iT,:)	= 0;                 	% Tris dont have diags.
            VertIDs	= mat2cell([VertIDs(1:2:end,:) VertIDs(2:2:end,:)],...
                ones(length(VertIDs)/2,1),[2 2]); 	% Convert to a cell.
        end
        ...
        function EL	= edgeLength(CM,EdgeIDs)
            ...
            [X,Y]	= edgeCoordinates(CM,EdgeIDs(:));
        ...
        function [X,Y,Z]    = edgeMidpoint(CM,EdgeIDs)
            ...
            [X,Y,Z]   = edgeCoordinates(CM,EdgeIDs);% Edge's coordinates.
        ...
        function edgeIDs    = edgeOrder(CM,VertIDs)
            ...
            % Get edges of each VertID, then ElemIDs of each EdgeID.
            edgeIDs	= CM.vert2Edge('Store','m','VertIDs',VertIDs);
            ElemIDs = reshape(CM.edge2Elem(edgeIDs')',size(edgeIDs,2)*2,numel(VertIDs))'
            EdgeIDs = edgeIDs;                      % Could do this with a map?
            EdgeIDs(:,2:end)	= 0
            for idx = 1:size(edgeIDs,1)
                % Check each following edge.
                irows	= 1:numel(VertIDs);
                for jdx	= idx+2:2:size(ElemIDs,1)-2
                    nextEdge	= ElemIDs(irows,jdx:jdx+1)
                    adjacentEdges	= ismember(nextEdge,ElemIDs(:,jdx-1))
                    if any(adjacentEdges)
                        iadjacentEdges = sum(adjacentEdges,2) > 0
                        EdgeIDs(iadjacentEdges,2) = nextEdge(adjacentEdges)
                        irows = ~iadjacentEdges
                    end
                end
            end
            ...
            %             ElemIDs = reshape(CM.edge2Elem(EdgeIDs')',size(EdgeIDs,2)*2,numel(VertIDs))';
            %             for idx = 1:size(EdgeIDs,2)-2
            %                 jdx = idx + 1;
            %                 idxElemIDs = CM.edge2Elem(EdgeIDs(:,idx));
            %                 rotatedColumns = false(numel(VertIDs),1);
            %                 cont = true;
            %                 while cont
            %                     nextElemIDs = CM.edge2Elem(EdgeIDs(:,jdx));
            %                     idx0 = sum(nextElemIDs > 0,2) == 0;
            %                     if any(idx0)
            %                         nextElemIDs(idx0,:)	= NaN;  % Ignore zeros til end.
            %                     end
            %
            %                     adjacentEdges = sum(ismember(idxElemIDs,nextElemIDs),2) > 0;
            %                     if any(adjacentEdges & ~rotatedColumns)
            %                         rotatedColumns(adjacentEdges) = true;
            %                         if jdx == idx + 1           % No edge rotation needed.
            %                             % Do nothing with these edges.
            %                             jdx	= jdx + 1;      	% Move to next idx.
            %                         else                        % Edge jdx becomes adjacent to idx.
            %                             EdgeIDs(adjacentEdges,idx+1:end) = EdgeIDs(adjacentEdges,[jdx:end,idx+1:jdx-1]);
            %                         end
            %                     else                            % Edges are not adjacent.
            %                         jdx	= jdx + 1;              % Move to next edge jdx.
            %                     end
            %                     if jdx > size(EdgeIDs,2) || all(rotatedColumns)
            %                         cont = false;
            %                     end
            %                 end
            %             end
        ...
        function EdgeIDs	= elem2Edge(CM,ElemIDs)
            ...
            idx0	= ElemIDs == 0;                 % Search for zeros.
            EdgeIDs = zeros(numel(ElemIDs),3,'uint32');
        ...
        function [X,Y,Z]	= elemCoordinates(CM,ElemIDs)
            ...
            idx0    = ElemIDs == 0;                 % Search for zeros.
            VertIDs = zeros(numel(ElemIDs),3);      % Get verts of elems.
            VertIDs(~idx0,:)	= CM.Adjacencies.Elem2Vert(ElemIDs(~idx0),:);
            [X,Y,Z]	= CM.vertCoordinates(VertIDs);  % Get coordinates.
        ...
        function [Quality,Theta]	= elemQuality(CM,varargin)
            ...
            [iT,iQ] = elemType(CM,p.Results.ElemIDs);
            
            % Compute quality.
            switch p.Results.Quality
                case {'skew','skewness','angular skewness'}
                    % Compute the maximum and minimum interior angles.
                    Theta	= interiorAngles(CM,p.Results.ElemIDs);
                    Tmax    = max(Theta(iT,end-2:end),[],2);% Max tri angles.
                    Tmin	= min(Theta(iT,end-2:end),[],2);% Min tri angles.
                    Qmax    = max(Theta(iQ,:),[],2);% Max quad angles.
                    Qmin	= min(Theta(iQ,:),[],2);% Min quad angles.
                    
                    % Compute the equiangle skew of each element.
                    Quality = zeros(length(p.Results.ElemIDs),1);
                    Quality(iT)	= 1-max((Tmax-60)./(180-60),(60-Tmin)./60);
                    Quality(iQ)	= 1-max((Qmax-90)./(180-90),(90-Qmin)./90);
        ...
        function [iT,iQ]	= elemType(CM,ElemIDs)
            ...
            ARG	= nargin;
            if ARG == 1
                % Do not perform operation if all elements are tris.
            ...
            % Index to all triangular faces in C.
            idx0	= true(length(ElemIDs),1);      % Search for zeros.
            idxTris = idx0;                         % Search for tris.
            idx0(ElemIDs ~= 0)	= false;
            idxTris(idx0)   = false;               	% Ignore zero indices.
            switch size(CM.ConnectivityList(ElemIDs(~idx0),:),2)
                case 3                              % Only tris exist.
                    iT  = ElemIDs(~idx0);
                    
                case 4                              % Quads exist.
                    idxTris(CM.ConnectivityList(ElemIDs(~idx0),4) == 0)	= 0;
                    iT	= find(idxTris);            % Tris in mesh.
                    
                otherwise
                    errordlg('Element type must be triangular or quadrilateral.','Invalid Element Type');
            end
            
            % Index to all quadrilaterals in the mesh.
            if nargout == 2
                iQ	= find(~idxTris & ~idx0);
            end
        end
        ...
        function Theta	= interiorAngles(CM,ElemIDs)
            ...
            % Discretize elements in mesh by element by type.
            idx0 = ElemIDs == 0;                    % Search for zeros.
            nzElemIDs = ElemIDs(~idx0);
            [iT,iQ]	= elemType(CM,nzElemIDs);
            
            % For triangles: Compute edge lengths then interior angles.
            if ~isempty(iT)                         % Tris exist in mesh.
                % Compute edge lengths.
                EdgeIDs = elem2Edge(CM,iT)';
                EL  = zeros(size(EdgeIDs));     	% Edge lengths.
                EL(nonZero)	= edgeLength(CM,EdgeIDs);
                
                % Compute interior angles from edge lengths.
                Theta   = zeros(length(nzElemIDs),size(CM.ConnectivityList,2));
                Theta(ismember(ElemIDs,iT),1:3)	= [...
            ...
            if ~isempty(iQ)
                ...
                iQb	= ismember(ElemIDs,iQ);         % Boolean to ElemIDs.
                for idx	= 2:5
                    ...
                    Theta(iQb,idx-1)	= real(acosd((L1.^2 +...
                        L2.^2 - L3.^2)./(2*L1.*L2)));
                end
            end
        end
        ...
        function [iCW,CM,A]	= isPolyCCW(CM,varargin)
            ...
            defaultElemIDs  = zeros(CM.nElems,1,'uint32');
            defaultElemIDs(:)	= 1:CM.nElems;   	% All elements.
            allowableIndex	= {'index','logical','Index','Logical'};
            addRequired(p,'CM',@isCHILmesh);
            ...
            A	= signedArea(CM,'signed',p.Results.ElemIDs);
            ...
            if nargout > 1                          % Rotate about node 1.
                CM.ConnectivityList(iCW,:)  = [CM.ConnectivityList(iCW,1),...
                    fliplr(CM.ConnectivityList(iCW,2:end))];
            end
        end
        ...
        function VertIDs	= medians(CM,EdgeIDs)
            ...
            vertsOfEdgeIDs = edge2Vert(CM,EdgeIDs);
            
            % Elements attached to EdgeIDs.
            ElemIDs	= edge2Elem(CM,EdgeIDs);
...
%             set(CM,'nVerts',nVerts);                % Update CM accordingly.
        ...
        % Signed Area
        function A	= signedArea(CM,varargin)
            %SIGNEDAREA Signed area of triangular or quad element(s).
            %   A = SIGNEDAREA(CM) for mesh class CM returns the signed
            %   area of each element in the mesh. A is a Cx1 vector, where
            %   C is the number of elements in the mesh.
            %
            %   NOTE:
            %       The signed area A of a planar non-self-intersecting,
            %       convex polygon is computed from its vertices.
            %       A negative value of A indicates a clockwise (CW)
            %       orientation of the polygon's vertices while a positive
            %       value indicates counter-clockwish (CCW) orientation.
            %
            %   A = SIGNEDAREA(CM,ElemIDs) returns the areas of elements
            %   in the mesh corresponding to the Cx1 or 1xC vector ElemIDs.
            %
            %   A = SIGNEDAREA(CM,SIGN) specifies whether A is signed or
            %   unsigned, where the default value of SIGN = 'signed' and
            ...
            C   = CM.ConnectivityList(p.Results.ElemIDs,:);
            [iT,iQ]	= elemType(CM,p.Results.ElemIDs);
            
            % Compute determinant (x1y2 - x2y1 + ..) and signed area (0.5*det).
            A	= zeros(size(C,1),1);               % Area(s).
            A(iT) = .5*(CM.Points(C(iT,1),1).*CM.Points(C(iT,2),2) -...
                CM.Points(C(iT,2),1).*CM.Points(C(iT,1),2) +...
                CM.Points(C(iT,2),1).*CM.Points(C(iT,3),2) -...
                CM.Points(C(iT,3),1).*CM.Points(C(iT,2),2) +...
                CM.Points(C(iT,3),1).*CM.Points(C(iT,1),2) -...
                CM.Points(C(iT,1),1).*CM.Points(C(iT,3),2));
            A(iQ) = .5*(...
                CM.Points(C(iQ,1),1).*CM.Points(C(iQ,2),2) -...
                CM.Points(C(iQ,2),1).*CM.Points(C(iQ,1),2) +...
                CM.Points(C(iQ,2),1).*CM.Points(C(iQ,3),2) -...
                CM.Points(C(iQ,3),1).*CM.Points(C(iQ,2),2) +...
                CM.Points(C(iQ,3),1).*CM.Points(C(iQ,4),2) -...
                CM.Points(C(iQ,4),1).*CM.Points(C(iQ,3),2) +...
                CM.Points(C(iQ,4),1).*CM.Points(C(iQ,1),2) -...
                CM.Points(C(iQ,1),1).*CM.Points(C(iQ,4),2));
            
            % Output unsigned area, if specifed.
            if strcmpi(p.Results.Sign,{'unsign','unsigned'})
                A   = abs(A);                   	% Unsigned area.
            end
        end
        ...
        function EdgeIDs	= vert2Edge(CM,varargin)
            ...
            if any(~strcmpi(p.Results.Store,{'Sparse','sparse','s'}))
                edgeIDs = sort(edgeIDs.');          % Presort and working by col to improve speed.
                [~,Vert,Edge]	= find(edgeIDs); 	% Find nonzeros.
                uVA	= unique([Vert,Edge],'rows');	% Unique edge per vert.
                numOfEdges	= diff([0; find(diff(uVA(:,1))); size(uVA,1)]);
                EdgeIDs	= mat2cell(uVA(:,2),numOfEdges);
                % Could improve ^ by breaking matrix into chunks and using PARFOR
        ...
        function ElemIDs	= vert2Elem(CM,varargin)
            ...
            % Adjust output, if necessary.
            if any(~strcmpi(p.Results.Store,{'Sparse','sparse','s'}))
                elemIDs = sort(elemIDs.');          % Presort and working by col to improve speed.
                [~,Vert,Elem]	= find(elemIDs); 	% Find nonzeros.
                uVA	= unique([Vert,Elem],'rows');   % Unique attach per v.
                tmp	= diff([0; find(diff(uVA(:,1))); size(uVA,1)]);
                ElemIDs(~idx0)	= mat2cell(uVA(:,2),tmp);
                % Could improve ^ by breaking matrix into chunks and using PARFOR
        ...
        function Ax	= axisCHILmesh(CM)
            ...
            if ~isempty(get(Ax,'Children'))         % Axis is open.
                ...
                % Compute axis limits.
                xlim(1) = min(CM.Points(CM.ConnectivityList,1));
                xlim(2) = max(CM.Points(CM.ConnectivityList,1));
                ylim(1) = min(CM.Points(CM.ConnectivityList,2));
                ylim(2)	= max(CM.Points(CM.ConnectivityList,2));
        ...
        function ph	= plot(CM,varargin)
            ...
            %
            %   See also CHILMESH, TRIPLOT, CHILMESH/EDGE,
            ...
            axisCHILmesh(CM);                       % Compute offset.
        ...
        function ph	= plotEdge(CM,varargin)
            ...
            %
            %   See also CHILMESH, CHILMESH/PLOT, CHILMESH/PLOTPOINT.
            %   CHILMESH/PLOTLAYER.
            %==============================================================
            ...
            VertIDs	= edge2Vert(CM,EdgeIDs)';
            
            % Create x, y coordinate vectors with NaNs.
            axisCHILmesh(CM);                       % Compute offset, limits.
            ...
            axisCHILmesh(CM);   hold off;           % Daspect, etc.
        end
            ...
            switch p.Results.PointType
                case {'Element','element','Elements','elements','Elem',...
                    ...
                    if strcmpi(p.Results.PointType,{'Element','element',...
                        ...
                        [X,Y]   = elemCoordinates(CM,ElemIDs);
                        
                    else                            % Plot Centroid.
                        [X,Y]	= centroid(CM,ElemIDs);
                    end
                ...
                case {'Vertex','vertex','Vertices','vertices','Vert','vert','Verts','verts',...
                    ...
                    [X,Y]	= vertCoordinates(CM,VertIDs);
